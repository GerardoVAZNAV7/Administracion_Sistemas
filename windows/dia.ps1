...
# # $LocalUserPath = "C:\FTP\LocalUser"
# # $FtpRootPath   = "C:\FTP"
# # $Username      = "bills"   # <-- cambia si el usuario tiene otro nombre

# # # Detectar grupo actual del usuario
# # $grupo = ""
# # try {
# #     if (Get-LocalGroupMember -Group "Reprobados"   -ErrorAction SilentlyContinue |
# #         Where-Object { $_.Name -like "*$Username" }) { $grupo = "Reprobados" }
# #     elseif (Get-LocalGroupMember -Group "Recursadores" -ErrorAction SilentlyContinue |
# #         Where-Object { $_.Name -like "*$Username" }) { $grupo = "Recursadores" }
# # } catch {}

# # Write-Host "Usuario  : $Username" -ForegroundColor Cyan
# # Write-Host "Grupo    : $(if ($grupo) { $grupo } else { 'NO ENCONTRADO' })" -ForegroundColor Cyan

# # $adminSID = (New-Object System.Security.Principal.SecurityIdentifier("S-1-5-32-544")).Translate(
# #                 [System.Security.Principal.NTAccount]).Value

# # $userRoot    = "$LocalUserPath\$Username"
...
# # # ── 2. Tomar ownership y resetear permisos ────────────────────────────────────
# # Write-Host ""
# # Write-Host "Aplicando permisos NTFS..." -ForegroundColor Yellow

# # takeown /F $userRoot    /R /D Y 2>$null | Out-Null
# # takeown /F $personalDir /R /D Y 2>$null | Out-Null

# # # userRoot — IIS verifica aqui antes de hacer login
# # icacls $userRoot /inheritance:r                        | Out-Null
# # icacls $userRoot /grant "SYSTEM:(OI)(CI)F"             | Out-Null
# # icacls $userRoot /grant "${adminSID}:(OI)(CI)F"        | Out-Null
# # icacls $userRoot /grant "IUSR:(OI)(CI)RX"              | Out-Null   # critico
# # icacls $userRoot /grant "IIS_IUSRS:(OI)(CI)RX"         | Out-Null   # critico
# # icacls $userRoot /grant "${Username}:(OI)(CI)RX"       | Out-Null
# # Write-Host "  [OK] userRoot permisos aplicados" -ForegroundColor Green

# # # personalDir — carpeta home visible para el usuario
# # icacls $personalDir /inheritance:r                     | Out-Null
# # icacls $personalDir /grant "SYSTEM:(OI)(CI)F"          | Out-Null
# # icacls $personalDir /grant "${adminSID}:(OI)(CI)F"     | Out-Null
# # icacls $personalDir /grant "IUSR:(RX)"                 | Out-Null
# # icacls $personalDir /grant "IIS_IUSRS:(RX)"            | Out-Null
# # icacls $personalDir /grant "${Username}:(OI)(CI)M"     | Out-Null   # Modify recursivo
# # Write-Host "  [OK] personalDir permisos aplicados" -ForegroundColor Green

# # # ── 3. Recrear symlinks ───────────────────────────────────────────────────────
# # Write-Host ""
# # Write-Host "Recreando symlinks..." -ForegroundColor Yellow

# # $generalTarget = "C:\FTP\LocalUser\Public\General"
# # $generalLink   = "$userRoot\General"

# # # Eliminar symlink viejo si existe
# # if (Test-Path $generalLink) {
# #     $item = Get-Item $generalLink -Force -ErrorAction SilentlyContinue
# #     if ($item.Attributes -band [IO.FileAttributes]::ReparsePoint) {
# #         cmd /c "rmdir `"$generalLink`"" 2>$null | Out-Null
...
# #     }
# # }
# # New-Item -ItemType SymbolicLink -Path $generalLink -Target $generalTarget -ErrorAction SilentlyContinue | Out-Null
# # if (-not (Test-Path $generalLink)) {
# #     cmd /c "mklink /D `"$generalLink`" `"$generalTarget`"" | Out-Null
# # }
# # Write-Host "  [OK] Symlink General -> $generalTarget" -ForegroundColor Green

# # # Symlink de grupo
# # if ($grupo) {
# #     $groupLink   = "$userRoot\$grupo"
# #     $groupTarget = "$FtpRootPath\$grupo"
# #     if (Test-Path $groupLink) {
# #         $item = Get-Item $groupLink -Force -ErrorAction SilentlyContinue
# #         if ($item.Attributes -band [IO.FileAttributes]::ReparsePoint) {
# #             cmd /c "rmdir `"$groupLink`"" 2>$null | Out-Null
# #         } else {
# #             Remove-Item $groupLink -Recurse -Force -ErrorAction SilentlyContinue
# #         }
# #     }
# #     New-Item -ItemType SymbolicLink -Path $groupLink -Target $groupTarget -ErrorAction SilentlyContinue | Out-Null
# #     if (-not (Test-Path $groupLink)) {
# #         cmd /c "mklink /D `"$groupLink`" `"$groupTarget`"" | Out-Null
# #     }
# #     Write-Host "  [OK] Symlink $grupo -> $groupTarget" -ForegroundColor Green
# # }

# # # ── 4. Verificar permisos en LocalUserPath (padre) ───────────────────────────
# # Write-Host ""
# # Write-Host "Verificando permisos en LocalUserPath padre..." -ForegroundColor Yellow
# # icacls $LocalUserPath /grant "IUSR:(RX)"        | Out-Null
# # icacls $LocalUserPath /grant "IIS_IUSRS:(RX)"   | Out-Null
# # icacls $LocalUserPath /grant "Users:(RX)"        | Out-Null
# # Write-Host "  [OK] LocalUserPath permisos actualizados" -ForegroundColor Green

# # # ── 5. Reiniciar FTP ──────────────────────────────────────────────────────────
# # Write-Host ""
# # Write-Host "Reiniciando servicio FTP..." -ForegroundColor Yellow
# # Stop-Service ftpsvc -Force -ErrorAction SilentlyContinue
# # Start-Sleep -Seconds 2
# # Start-Service ftpsvc -ErrorAction SilentlyContinue
# # Start-Sleep -Seconds 1
...
# # } else {
# #     Write-Host "  [!] ftpsvc no arranco, revisa el Visor de Eventos." -ForegroundColor Red
# # }

# # # ── 6. Diagnóstico final ──────────────────────────────────────────────────────
# # Write-Host ""
# # Write-Host "=== DIAGNOSTICO FINAL ===" -ForegroundColor Cyan
# # Write-Host "userRoot    : $(Test-Path $userRoot)    -> $userRoot"
# # Write-Host "personalDir : $(Test-Path $personalDir) -> $personalDir"
# # Write-Host "generalLink : $(Test-Path $generalLink) -> $generalLink"
# # if ($grupo) {
# #     Write-Host "groupLink   : $(Test-Path $groupLink)  -> $groupLink"
# # }
# # Write-Host ""
...
# Eliminar sitio FTP existente
Import-Module WebAdministration
Remove-WebSite -Name "FTP" -ErrorAction SilentlyContinue
