function Instalar-ServicioFTP {
    Write-Host "[+] Instalando IIS y Servicio FTP..." -ForegroundColor Cyan
    ...
    if (Get-Service ftpsvc -ErrorAction SilentlyContinue) {
        ...
        Write-Host "[X] Error: Ejecuta como Administrador." -ForegroundColor Red
    }
}

function Configurar-EntornoFTP {
    Write-Host "[+] Configurando directorios y permisos..." -ForegroundColor Cyan
