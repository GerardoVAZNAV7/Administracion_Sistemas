...
while ($true) {
    ...
    # Validar que no esté vacía ni tenga caracteres inválidos
    if ([string]::IsNullOrWhiteSpace($opcion) -or $opcion -notmatch '^[1-5]$') {
        Write-Host "  [!] Opción inválida. Elige entre 1 y 5." -ForegroundColor Yellow
        continue
    }
    ...
    Write-Host ""
    Write-Host "  ─────────────────────────────────────────────" -ForegroundColor DarkGray
    Write-Host "  Verificación desde tu máquina host:" -ForegroundColor Cyan
